...
async function subtitlesHandler({ type, id, extra, config }) {
  ...
  try {
    ...
    if (selectablePairs.length > 0) {
      finalSubtitles = selectablePairs.map(pair => {
        const dynamicParams = [
          ...
          mainLang,
          transLang,
          encodeURIComponent(pair.main.id),
          encodeURIComponent(pair.trans.id)
        ].join('/');
    ...
    debugServer.log(`Publishing ${finalSubtitles.length} dual subtitle track option(s)`);
...
async function generateDynamicSubtitle(
  ...
  try {
    ...
    const transSub = allSubtitles.find(s => String(s.id) === String(decodedTransId));
    ...
    // Try candidate pairs from aggregated scrapers
    ...
    // Fallback single-track rendering
